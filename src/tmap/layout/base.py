...
# =============================================================================
# EXAMPLE: Minimal radial layout (for reference)
# =============================================================================

class _RadialLayoutExample(Layout):
    """
    Simple radial layout - FOR REFERENCE ONLY.

    This shows the minimum you need to implement.
    Not recommended for actual use (doesn't respect distances).
    """

    def _compute_initial(self, tree: Tree) -> Coordinates:
        """Place nodes in concentric circles by depth."""
        n = tree.n_nodes
        ...
        for node, parent, depth in tree.bfs():
            if depth == 0:
                ...
                # Place on circle at radius = depth
                # Angle based on position among siblings
                siblings = tree.children(parent, None) if parent is not None else []
                idx = siblings.index(node) if node in siblings else 0
                n_siblings = len(siblings) or 1

                angle = 2 * np.pi * idx / n_siblings
                radius = depth * 0.5

                x[node] = radius * np.cos(angle)
                y[node] = radius * np.sin(angle)
    ...
    def _compute_incremental(
        ...
        """For radial, just recompute everything (it's fast)."""
